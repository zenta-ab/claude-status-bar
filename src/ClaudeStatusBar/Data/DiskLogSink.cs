...
///
/// Retention (Codex review Medium #10): raw daily logs are capped at 20MB/file and
/// kept 7 days; window-shape.csv is rotated monthly (window-shape-YYYY-MM.csv) and
/// kept 6 months -- see CsvReplay.ReadRowsForWarmStart for the reader side.
/// </summary>
public sealed class DiskLogSink : IAsyncDisposable
{
    ...
    const int CsvRetentionMonths = 6;
    ...
    async Task RunAsync()
    {
        ...
        while (true)
        {
            if (DrainOnce(rawReader, csvReader)) continue;
            if (rawReader.Completion.IsCompleted && csvReader.Completion.IsCompleted) break;

            try
            {
                Task rawWait = rawReader.WaitToReadAsync(_cts.Token).AsTask();
                Task csvWait = csvReader.WaitToReadAsync(_cts.Token).AsTask();
                await Task.WhenAny(rawWait, csvWait).ConfigureAwait(false);
            }
        ...
        DrainOnce(rawReader, csvReader); // best-effort final flush; caller bounds total wait via DisposeAsync's deadline
    }
    ...
    void SweepRetentionOnce()
    {
        ...
        try
        {
            ...
            DateTime csvCutoff = DateTime.UtcNow.AddMonths(-CsvRetentionMonths);
